    ...
    def post(self):
        ...
        with connect() as db:
            ...
            try:
                ...
                if appliances:
                    ...
                    for appliance in appliances:
                        ...
                        # print(newRequestID)
                        NewApplianceID = db.call("new_appliance_uid")["result"][0][
                            "new_id"
                        ]
                        ...
                        response["Add Appliance"] = db.insert(
            ...
            payload["property_images"] = (
                "[]"
                if payload.get("property_images") in {None, "", "null"}
                else payload.get("property_images", "[]")
            )
            print("Add Property Payload: ", payload)
            payload["property_listed_date"] = (
                datetime.today().strftime("%m-%d-%Y")
                if payload.pop("property_listed") == "1"
                else ""
            )
...
class PropertyOwners(Resource):
    def get(self, property_id=None):
        print("In PropertyOwners GET")
        response = {}
        ...
        if owner_email:
            ...
            with connect() as db:
                # Get all property ownership info based on email
                propertyOwnershipQuery = db.execute(
                    """
                    -- GET ALL PROPERTY OWNERSHIP INFO BY OWNER EMAIL
                    SELECT 
                        po.property_id,
                        ...
                        po.po_start_date,
                        po.po_end_date
                    FROM property_owner AS po
                    LEFT JOIN ownerProfileInfo AS opi ON po.property_owner_id = opi.owner_uid
                    WHERE opi.owner_email = \'"""
                    + owner_email
                    + """\'
                    """
                )

                response["result"] = propertyOwnershipQuery.get("result", [])
                response["count"] = len(response["result"])

                if response["count"] == 0:
                    response["message"] = (
                        f"No property ownership found for email: {owner_email}"
                    )
                    return response, 404

                response["message"] = (
                    "Property ownership data retrieved successfully by email"
                )
                return response, 200
            ...
            with connect() as db:
                # Get all property ownership info for a specific property with owner email
                propertyOwnershipQuery = db.execute(
                    """
                    -- GET ALL PROPERTY OWNERSHIP INFO FOR SPECIFIC PROPERTY
                    SELECT 
                        po.property_id,
                        po.property_owner_id,
                        po.po_owner_percent,
                ...
                if response["count"] == 0:
                    response["message"] = (
                        f"No property ownership found for property_id: {property_id}"
                    )
                    return response, 404

                response["message"] = "Property ownership data retrieved successfully"
                return response, 200
        else:
            response["message"] = (
                "Please provide either property_id or owner_email parameter"
            )
            return response, 400
...
# we are searching for specific address


class PropertiesSearch(Resource):
    def get(self):
        response = {}
        property_address = request.args.get("property_address")
        property_unit = request.args.get("property_unit", "")
        property_city = request.args.get("property_city")
        property_state = request.args.get("property_state")
        property_zip = request.args.get("property_zip")

        print(
            f"Get request : property search Address: {property_address}, Unit: '{property_unit}', City: {property_city}, State: {property_state}, Zip: {property_zip}"
        )

        if not all([property_address, property_city, property_state, property_zip]):
            return {
                "message": "please provide address, address ,zip, city,state",
                "code": 400,
            }, 400
        ...
        with connect() as db:
            query = f"""
                    ...
                    WHERE property_address='{property_address}'
                        ...
                        AND property_state='{property_state}'
                        AND property_zip= '{property_zip}'
            ...
            response["count"] = len(response["properties"])
            return response
