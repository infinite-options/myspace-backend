...
class Listings (Resource):
    def get(self,tenant_id):
        ...
        with connect() as db:
            ...
            #         LEFT JOIN ( 
            #             SELECT * FROM u_details                                                 
            #             ) AS u                                       
            #         ON utility_property_id = property_uid
            #         LEFT JOIN ( 
            #             SELECT * FROM leases 
            #             -- WHERE lease_status = "ACTIVE"
            #             WHERE (lease_status IN ('ACTIVE', 'ACTIVE M2M')) AND STR_TO_DATE(lease_end, '%m-%d-%Y') > DATE_ADD(CURDATE(), INTERVAL 2 MONTH)
            #             ) AS l
            ...
            listingsQuery = db.execute(""" 
                    ...
                    SELECT * 
                    FROM properties
                    LEFT JOIN (SELECT * FROM contracts WHERE contract_status = 'ACTIVE') c ON property_uid = contract_property_id
                    LEFT JOIN u_details ON utility_property_id = property_uid
