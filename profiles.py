...
class Profile(Resource):
    ...
    def post(self):
        ...
        with connect() as db:
            if owner:
                print("owner")
                profile_info["owner_uid"] = db.call("new_owner_uid")["result"][0][
                    "new_id"
                ]
                ...
                profile_info["owner_documents"] = (
                    "[]"
                    if profile_info.get("owner_documents") in {None, "", "null"}
                    else profile_info.get("owner_documents", "[]")
                )
                ...
                profile_info["tenant_documents"] = (
                    profile_info["tenant_documents"]
                    if "tenant_documents" in profile_info
                    else "[]"
                )
                profile_info["tenant_adult_occupants"] = (
                    profile_info["tenant_adult_occupants"]
                    if "tenant_adult_occupants" in profile_info
                    else "[]"
                )
                profile_info["tenant_children_occupants"] = (
                    profile_info["tenant_children_occupants"]
                    if "tenant_children_occupants" in profile_info
                    else "[]"
                )
                profile_info["tenant_vehicle_info"] = (
                    profile_info["tenant_vehicle_info"]
                    if "tenant_vehicle_info" in profile_info
                    else "[]"
                )
                profile_info["tenant_references"] = (
                    profile_info["tenant_references"]
                    if "tenant_references" in profile_info
                    else "[]"
                )
                profile_info["tenant_pet_occupants"] = (
                    profile_info["tenant_pet_occupants"]
                    if "tenant_pet_occupants" in profile_info
                    else "[]"
                )
                profile_info["tenant_employment"] = (
                    profile_info["tenant_employment"]
                    if "tenant_employment" in profile_info
                    else "[]"
                )
                # print("Updated Tenant Profile: ", tenant_profile)

                profile_info["tenant_uid"] = db.call("new_tenant_uid")["result"][0][
                    "new_id"
                ]
                ...
                print("manager")
                profile_info["business_uid"] = db.call("new_business_uid")["result"][0][
                    "new_id"
                ]
                ...
                profile_info["business_documents"] = (
                    "[]"
                    if profile_info.get("business_documents") in {None, "", "null"}
                    else profile_info.get("business_documents", "[]")
                )
                response = db.insert("businessProfileInfo", profile_info)
                # response["business_uid"] = profile_info["business_uid"]

                print("employee")
                employee_info["employee_uid"] = db.call("new_employee_uid")["result"][
                    0
                ]["new_id"]
    ...
    def put(self):
        ...
        if payload.get("tenant_uid"):
            ...
            valid_columns = {
                ...
                "business_zip",
                "business_photo_url",
                "business_documents_details",
                "delete_documents",
            }
            filtered_payload = {
                key: value for key, value in payload.items() if key in valid_columns
            }
        ...
        with connect() as db:
            ...
            if payload.get("employee_uid"):
                response["employee"] = db.update(
                    "employees", employee_key, employee_payload
                )
