...
#             # print("Item Payment Query: ", paymentQuery)
#             response['payment'] = db.execute(paymentQuery, [], 'post')

#             # FOR EACH PURCHASE

#             purchase_ids = data['pay_purchase_id']
#             num_purchases = len(purchase_ids)
#             print("number of items: ", num_purchases)
...
#                 # PUT EACH PAYMENT IN PAYMENTS_PURCHASE TABLE

#                 # GET NEW PURCHASE_PAYMENT UID
#                 newPPRequestID = db.call('new_purchase_payment_uid')['result'][0]['new_id']
#                 print(newPPRequestID)

#                 ppQuery = (""" 
#                     -- INSERT ITEM INTO PURCHASE_PAYMENT TABLE
#                     INSERT INTO space.purchase_payment
#                     SET pp_uid = \'""" + newPPRequestID + """\'
#                         , pp_purchase_id = \'""" + item['purchase_uid'] + """\'
#                         , pp_payment_id = \'""" + newPaymentRequestID + """\'
#                         , pp_timestamp = DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 8 HOUR), '%m-%d-%Y %H:%i')
#                         , pp_amount = \'""" + item['pur_amount_due'] + """\'
#                     """)

#                 print("Item Payment Query: ", ppQuery)
#                 response = db.execute(ppQuery, [], 'post')
