    ...
    def put(self):
        ...
        if payload.get('maintenance_quote_uid'):
            if payload.get('quote_status') == 'REQUESTED':
                with connect() as db:
                    maintenance_quote = db.select('space_dev.maintenanceQuotes', {'maintenance_quote_uid': payload.get('maintenance_quote_uid')})
                    maintenance_request_uid = maintenance_quote.get('result')[0]['quote_maintenance_request_id']
                    response = db.update('space_dev.maintenanceRequests',
            ...
            with connect() as db:
                response_2 = db.update('space_dev.maintenanceQuotes',
            ...
            if payload.get('maintenance_request_status') == 'CANCELLED':
                with connect() as db:
                    ...
                    response = db.update('space_dev.maintenanceQuotes',
                                         {'quote_maintenance_request_id': maintenance_request_uid},
                                         {'quote_status': 'WITHDRAWN'})
            with connect() as db:
                response_2 = db.update('space_dev.maintenanceRequests',
                                       {'maintenance_request_uid': payload.get('maintenance_request_uid')},
