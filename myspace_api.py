...
#                                     DATE_FORMAT(DATE_ADD(CONCAT(YEAR(NOW()), '-', MONTH(NOW()), '-', due_by), INTERVAL 2 WEEK), '%m-%d-%Y')
#                                 WHEN frequency = 'Annually' THEN 
#                                     DATE_FORMAT(DATE_ADD(CONCAT(YEAR(NOW()), '-', MONTH(NOW()), '-', due_by), INTERVAL 1 YEAR), '%m-%d-%Y')
#                                 ELSE 
#                                     NULL
#                             END AS next_due_date
#                         FROM space.leaseFees                                                   -- get actual lease fees and payment frequency
#                         LEFT JOIN space.leases ON fees_lease_id = lease_uid                    -- get lease parameters
...
#                         LEFT JOIN space.contracts ON lease_property_id = contract_property_id  -- to make sure contract is active
...
#                         print("Rent is Payable")

#                         # CHECK IF PURCHASE IS ALREADY IN DATABASE
...
#                         # print("Late Fee: ", response['result'][i]['late_fee'], type(response['result'][i]['late_fee']))
#                         late_fee = 0 if not response['result'][i]['late_fee'] else response['result'][i]['late_fee']
#                         print("late_fee: ", late_fee, type(late_fee))
                            
#                         # Check if perDay_late_fee is NONE
#                         # print("Per Day Late Fee: ", response['result'][i]['perDay_late_fee'])
#                         perDay_late_fee = 0 if not response['result'][i]['perDay_late_fee'] else response['result'][i]['perDay_late_fee']
#                         print("perDay_late_fee: ", perDay_late_fee, type(perDay_late_fee))

#                         # Check if late_by is NONE
#                         # print("Late By: ", response['result'][i]['late_by'])
#                         late_by = 1 if not response['result'][i]['late_by'] else response['result'][i]['late_by']
...
#                         responsible_percent = 1.0 if response['result'][i]['lt_responsibility'] is None else response['result'][i]['lt_responsibility']
#                         print("responsible_percent: ", responsible_percent, type(responsible_percent))
...
#                         # print("Purchase Parameters: ", i, contract_uid, property, tenant, owner, manager, fee_name)
...
#                         # newRequest['pur_timestamp'] = datetime.today().strftime("%m-%d-%Y %H:%M:%S")
