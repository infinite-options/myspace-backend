...
class Bills(Resource):
    ...
    def post(self):
        ...
        with connect() as db:
            newBillUID = db.call('space_dev.new_bill_uid')['result'][0]['new_id']
            ...
            response['Add Bill'] = db.insert('space_dev.bills', payload)
                ...
                for key, value in data_dict.items():
                    ...
                    if bill_utility_type == "maintenance": 
                        ...
                        queryResponse = (""" 
                            ...
                            LEFT JOIN space_dev.lists AS utility ON utility_type_id = utility.list_uid
                            LEFT JOIN space_dev.lists AS payer ON utility_payer_id = payer.list_uid
                            LEFT JOIN (SELECT * FROM space_dev.leases WHERE lease_status = 'ACTIVE' OR lease_status = 'ACTIVE M2M') AS l ON utility_property_id = lease_property_id
                            LEFT JOIN space_dev.property_owner ON utility_property_id = property_id		-- TO FIND PROPERTY OWNER
                            LEFT JOIN space_dev.contracts ON utility_property_id = contract_property_id    -- TO FIND PROPERTY MANAGER
                            LEFT JOIN space_dev.lease_tenant ON lease_uid = lt_lease_id				-- TO FIND TENANT IDS AND RESPONSIBILITY PERCENTAGES
                            ) AS rp
                    ...
                    if bill_utility_type == "maintenance":
                        ...
                        if bill_maintenance_quote_id[:3] == "900": 
                            ...
                            new_purchase_uid = db.call('space_dev.new_purchase_uid')['result'][0]['new_id'] 
    ...
    #     with connect() as db:
    #         print("In actual PUT")
    #         response = db.update('space_dev.bills', key, bills)
    #     return response
    ...
    def put(self):
        ...
        with connect() as db:
            ...
            response['bill_info'] = db.update('space_dev.bills', key, payload)
        ...
        if payload.get('bill_notes') not in {None, '', 'null'} or payload.get('bill_amount') not in {None, '', 'null'}:
            ...
            with connect() as db:
                bill_purchase_query = db.execute(""" SELECT * FROM space_dev.purchases WHERE pur_bill_id = \'""" + key['bill_uid'] + """\' """)     # Current Purchase associated with Bill
                ...
                for purchase in bill_purchase_query["result"]:
                    ...
                    #  print(bill_pur_update)
                     ...
                     response['bill_update'] = db.update('space_dev.purchases', {'purchase_uid':purchase["purchase_uid"]}, bill_pur_update)
...
class AddPurchase(Resource):
    def post(self):
        ...
        with connect() as db:
            newPurchaseUID = db.call('space_dev.new_purchase_uid')['result'][0]['new_id']
           ...
           # Add Purachse Info
            ...
            response['Add Purchase'] = db.insert('space_dev.purchases', payload)
    ...
    def put(self):
        ...
        with connect() as db:
            print("In actual PUT")
            response = db.update('space_dev.purchases', key, purchases)
        return response
...
class AddPurchaseJSON(Resource):
    def post(self):
        ...
        with connect() as db:
            ...
            response = db.insert('space_dev.purchases', newRequest)
