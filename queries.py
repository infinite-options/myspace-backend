...
def testQuery(user_id):
    ...
    query = 'SELECT * FROM purchases WHERE {column} LIKE %s'
    # like_pattern = '600%'
...
def LeaseDetailsQuery(user_id):
    ...
    query = """
            ...
            LEFT JOIN u_details ON utility_property_id = lease_property_id
    ...
    try:
        ...
        with connect() as db:    
            response = db.execute("""
                    ...
                    FROM 
                        announcements a
                    LEFT JOIN businessProfileInfo b ON a.announcement_sender LIKE '600%' AND b.business_uid = a.announcement_sender
                    LEFT JOIN ownerProfileInfo c ON a.announcement_sender LIKE '110%' AND c.owner_uid = a.announcement_sender
                    LEFT JOIN tenantProfileInfo d ON a.announcement_sender LIKE '350%' AND d.tenant_uid = a.announcement_sender
                    WHERE 
    ...
    try:
        ...
        with connect() as db:    
            response = db.execute("""
					...
					FROM purchases
					LEFT JOIN contracts ON contract_property_id = pur_property_id
					LEFT JOIN property_owner ON property_id = pur_property_id
        ...
        with connect() as db:    
            response = db.execute("""
                    ...
                    FROM (
                        ...
                        LEFT JOIN lease_tenant ON fees_lease_id = lt_lease_id            	-- get tenant responsible for rent
                        LEFT JOIN property_owner ON lease_property_id = property_id      	-- get property owner and ownership percentage
                        LEFT JOIN (
                    ...
                    LEFT JOIN purchases ON lease_property_id = pur_property_id
...
def ContractDetails(user_id):
    ...
    query = """
                ...
                FROM o_details o
                LEFT JOIN properties p ON o.property_id =p.property_uid 
                LEFT JOIN b_details b ON o.property_id = b.contract_property_id
...
def MaintenanceRequests(user_id):
    ...
    query = """
            ...
            LEFT JOIN ( SELECT * FROM contracts WHERE contract_status = "ACTIVE") AS c ON maintenance_property_id = contract_property_id
...
def MaintenanceDetails(user_id):
    ...
    query = """
            ...
            FROM (
                SELECT * 
                FROM maintenanceRequests
                LEFT JOIN property_owner ON maintenance_property_id = property_id
                LEFT JOIN ( SELECT * FROM contracts WHERE contract_status = "ACTIVE") AS c ON maintenance_property_id = contract_property_id
...
def ProfileInfo(user_id):
    ...
    if user_id.startswith("110"):
        query = query.format(column='owner_uid', table = 'ownerProfileInfo')
    elif user_id.startswith("600"):
        query = query.format(column='business_uid', table = 'businessProfileInfo')
    elif user_id.startswith("350"):
        query = query.format(column='tenant_uid', table = 'tenantProfileInfo')
    else:
        print("Invalid condition type")
        return None
