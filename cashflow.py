    ...
    def get(self, user_id):
        ...
        with connect() as db:
            ...
            if user_id[:3] == '110':
                ...
                print("OWNER Cashflow by Year, by Month, by CF Type, by Purchase Category")
                cashflow = db.execute("""                            
                        -- DETAILS BY PROPERTY BY PURCHASE GROUP
                        SELECT 
                            property_uid, property_address, property_unit, property_city, property_state, property_zip,
                            pur_group, cf_month, cf_year,
                            SUM(IF(purchase_type = 'Management', pur_amount_due, 0)) AS total_amount_due,
                        ...
                        -- WHERE (pur_receiver = \'""" + user_id + """\' OR pur_payer = \'""" + user_id + """\')
                        AND pur_group IS NOT NULL 
                        GROUP BY property_uid, pur_group
                        """)
